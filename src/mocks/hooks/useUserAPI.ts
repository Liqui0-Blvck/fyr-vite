...
const useUserAPI = (username: string) => {
  ...
  const { response: users } = useAxiosFunction({
    method: "GET",
    url: "/auth/registro",
  });
  ...
  const allUserData: userData[] = users;
  console.log(allUserData);
  const userData = allUserData.filter((f) =>
    f.username.toLowerCase() === username
  );

  const { response: login } = useAxiosFunction({
    method: "POST",
    url: "/api/token/",
  });
